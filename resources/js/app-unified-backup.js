...
class ExportManager {
  ...
  handleCategoryChange(event) {
    ...
    // If this is a parent category with subcategories, handle them too
    if (category === 'lifeline') {
      const subcategoryCheckboxes = document.querySelectorAll('[data-parent-category="lifeline"]');
  ...
  async saveFavorite() {
    ...
    if (!name) {
      AppUtils.showToast('お気に入り名を入力してください。', 'error');
      return;
    }
    ...
    if (selectedFacilities.length === 0 || selectedFields.length === 0) {
      AppUtils.showToast('施設と出力項目を選択してください。', 'error');
      return;
    }
    ...
    try {
      ...
      AppUtils.showToast('お気に入りの保存に失敗しました。', 'error');
    }
  }
    ...
    try {
      ...
      if (data.success) {
        ...
        if (favoriteData.original_facility_count > favoriteData.accessible_facility_count) {
          AppUtils.showToast(`注意: ${favoriteData.original_facility_count - favoriteData.accessible_facility_count}件の施設にアクセス権限がないため、選択から除外されました。`, 'warning');
        }
      } else {
        AppUtils.showToast(data.message, 'error');
      }
      ...
      AppUtils.showToast('お気に入りの読み込みに失敗しました。', 'error');
    }
  }
  ...
  async editFavorite(id, currentName) {
    ...
    if (newName && newName.trim() !== '' && newName !== currentName) {
      try {
        ...
        AppUtils.showToast('お気に入りの更新に失敗しました。', 'error');
      }
    }
  }
  ...
  async deleteFavorite(id) {
    ...
    if (confirmed) {
      try {
        ...
        AppUtils.showToast('お気に入りの削除に失敗しました。', 'error');
      }
    }
  }
  ...
  handleFormSubmission(e) {
    ...
    console.log('ExportManager (app-unified): handleFormSubmission called');

    // Prevent duplicate submissions
    if (this.isSubmitting) {
      console.log('ExportManager (app-unified): Form submission already in progress, ignoring');
      return;
    }
    ...
    try {
      // Use CsvDownloadManager instead of direct form submission
      if (window.CsvDownloadManager) {
        console.log('ExportManager (app-unified): Using CsvDownloadManager');
        const downloadManager = new window.CsvDownloadManager();
        downloadManager.downloadCSV(selectedFacilities, selectedFields, exportButton);
      } else {
        console.log('ExportManager (app-unified): CsvDownloadManager not available, using fallback');
        AppUtils.showLoading(exportButton, '出力準備中...');
        this.downloadCSV(selectedFacilities, selectedFields, exportButton);
      }
    } catch (error) {
      console.error('ExportManager (app-unified): Form submission error:', error);
      AppUtils.showToast('CSV出力中にエラーが発生しました。', 'error');
      AppUtils.hideLoading(exportButton);
    } finally {
  ...
  downloadCSV(selectedFacilities, selectedFields, exportButton) {
    ...
    csrfToken.value = AppUtils.getCsrfToken();
    ...
    // Progress updates with shorter timeouts
    setTimeout(() => {
      AppUtils.showLoading(exportButton, 'CSV生成中...');
    }, 500);

    setTimeout(() => {
      AppUtils.showLoading(exportButton, 'ダウンロード準備中...');
    }, 1500);

    // Reset button state after a reasonable time
    setTimeout(() => {
      AppUtils.hideLoading(exportButton);
      AppUtils.showToast('CSVファイルのダウンロードを開始しました。', 'success');
...
class Application {
  ...
  async initializeModules() {
    ...
    // Initialize export module on CSV export pages
    if (currentPath.includes('/export/csv')) {
      console.log('Initializing CSV export management');
      await this.initializeExportManagement();
    }
  }
  ...
  async initializeExportManagement() {
    try {
      console.log('Looking for CSV export form...');
      const csvExportForm = document.getElementById('csvExportForm');
      console.log('CSV export form found:', !!csvExportForm);

      if (!csvExportForm) {
        console.log('CSV export form not found, skipping initialization');
        return;
      }
      ...
      console.log('ExportManager created:', exportManager);
      ...
      window.loadFavorite = (id) => exportManager.loadFavorite(id);
      window.editFavorite = (id, currentName) => exportManager.editFavorite(id, currentName);
      window.deleteFavorite = (id) => exportManager.deleteFavorite(id);

    } catch (error) {
      console.error('Failed to initialize export management:', error);
      ...
      if (csvExportForm) {
        ...
        errorDiv.innerHTML = `
          <h5>エラー</h5>
          <p>CSV出力機能の初期化に失敗しました。</p>
          <p><small>エラー詳細: ${error.message}</small></p>
          <button class="btn btn-outline-danger btn-sm" onclick="location.reload()">
            <i class="fas fa-refresh"></i> 再読み込み
          </button>
        `;
        csvExportForm.parentNode.insertBefore(errorDiv, csvExportForm);
      }
    }
  }
...
window.ShiseCal = {
  utils: AppUtils,
  api: new ApiClient(),
  createCsvDownloadManager: () => new CsvDownloadManager(),
  app: app,
  // Legacy functions
...
class CsvDownloadManager {
  ...
  setupIframeLoadHandler(context, exportButton) {
    ...
    context.iframe.addEventListener('load', () => {
      if (loadEventFired || context.downloadCompleted) {
      ...
      // Complete download after delay if no errors
      setTimeout(() => {
...
// Make CsvDownloadManager globally available
window.CsvDownloadManager = CsvDownloadManager;
