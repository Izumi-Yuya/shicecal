...
export class FacilityFormLayout {
   ...
   */
    ...
    // Create live region for form status (polite)
        ...
        if (!document.getElementById('progress-live-region')) {
   ...
   */
    addProgressIndicators() {
        ...
        progressContainer.setAttribute('aria-label', 'フォーム入力進捗');
        ...
        // Insert progress bar after facility info card or at form start
        const facilityCard = document.querySelector('.facility-info-card');
        const insertAfter = facilityCard || this.form.firstElementChild;
        ...
        this.form.addEventListener('change', () => this.updateFormProgress());
    }
   ...
   */
    updateFormProgress() {
        ...
        const allFields = this.form.querySelectorAll('input:not([type="hidden"]), select, textarea');
        ...
        progressBar.setAttribute('aria-label', `フォーム入力進捗 ${progress}%`);

        // Announce progress milestones
        if (progress === 25 || progress === 50 || progress === 75 || progress === 100) {
            this.announceToScreenReader(`フォーム入力が${progress}%完了しました`, 'polite');
        }
    }
