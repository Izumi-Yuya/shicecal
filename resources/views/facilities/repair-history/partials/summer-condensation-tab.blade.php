...
<div class="repair-history-equipment-sections">
    @if($summerCondensationHistory->isNotEmpty())
    {{-- 夏型対策セクション（上部） --}}
    @php
    $countermeasureHistory = $summerCondensationHistory->filter(function($history) {
        return in_array($history->subcategory, ['countermeasure', '夏型対策']);
    });
    @endphp

    @if($countermeasureHistory->isNotEmpty())
    ...
    {{-- 夏型結露対策履歴セクション（下部・テーブル形式） --}}
    @php
    $historyRecords = $summerCondensationHistory->filter(function($history) {
        return in_array($history->subcategory, ['history', '夏型結露対策履歴']);
    });
    
    // ID順でソートしてNo.用のマッピングを作成
    $historyRecordsById = $historyRecords->sortBy('id');
    $idToNoMapping = [];
    $no = 1;
    foreach ($historyRecordsById as $history) {
        $idToNoMapping[$history->id] = $no++;
    }
    
    // 表示用は施工日の古いものを上に
    $historyRecords = $historyRecords->sortBy('maintenance_date');
    @endphp

    @if($historyRecords->isNotEmpty())
    <div class="equipment-section mb-4">
        <h6 style="margin: 0 0 0.5rem 0; font-weight: bold; color: #333;">夏型結露対策履歴</h6>

        <div class="table-responsive {{ $historyRecords->count() > 6 ? 'summer-condensation-scrollable' : '' }}">
            <table class="table facility-basic-info-table-clean summer-condensation-table" style="--bs-table-cell-padding-x: 0; --bs-table-cell-padding-y: 0; margin-bottom: 0; table-layout: fixed; width: 100%; font-size: 0.75rem; border: none;">
                <thead>
    ...
    @php
        // 施設の夏型結露特記事項を取得
        $specialNotes = $facility->summer_condensation_special_notes ?? '';
    @endphp
            ...
            <tbody>
                <tr>
                    <td class="detail-value empty-field" style="padding: 0.5rem;">
                        <i class="fas fa-info-circle me-2"></i>夏型結露の修繕履歴が登録されていません。編集ボタンをクリックして履歴を登録してください。
                    </td>
                </tr>
            </tbody>
        </table>
...
<style>
/* 夏型結露対策履歴タブ専用スタイル - empty-fieldの背景色を強制的に透明にする */
.summer-condensation-table .detail-value {
    background-color: transparent !important;
}

.summer-condensation-table .detail-value.empty-field {
    background-color: transparent !important;
...
.summer-condensation-scrollable {
...
.summer-condensation-scrollable::-webkit-scrollbar {
    width: 8px;
}

.summer-condensation-scrollable::-webkit-scrollbar:horizontal {
    display: none;
}

.summer-condensation-scrollable::-webkit-scrollbar-track {
    background: #f1f1f1;
    border-radius: 4px;
}

.summer-condensation-scrollable::-webkit-scrollbar-thumb {
    background: #c1c1c1;
    border-radius: 4px;
}

.summer-condensation-scrollable::-webkit-scrollbar-thumb:hover {
    background: #a8a8a8;
}
...
/* テーブルの方向を元に戻す */
.summer-condensation-scrollable .table {
    direction: ltr;
    margin-left: 0;
}
</style>
...
document.addEventListener('DOMContentLoaded', function() {
    // スクロール可能なコンテナを取得
    const scrollableContainer = document.querySelector('.summer-condensation-scrollable');
