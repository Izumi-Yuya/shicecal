...
class CommonTableValidator
{
    ...
    private static function validateCellTypeSpecific(array $cellData, int $rowIndex, int $cellIndex): array
    {
        ...
        switch ($cellType) {
            ...
            case 'file_display':
                // file_display タイプは配列形式のファイル情報を期待
                if (is_array($value)) {
                    // 必要なキーの存在確認
                    $requiredKeys = ['filename', 'path', 'exists'];
                    foreach ($requiredKeys as $key) {
                        if (! isset($value[$key])) {
                            $warnings[] = "行 {$rowIndex}, セル {$cellIndex}: file_display に必要なキー '{$key}' がありません";
                        }
                    }

                    // ファイルパスの検証
                    if (isset($value['path']) && is_string($value['path']) && strpos($value['path'], '..') !== false) {
                        $errors[] = "行 {$rowIndex}, セル {$cellIndex}: 不正なファイルパスです";
                    }
                } elseif (! ValueFormatter::isEmpty($value)) {
                    $warnings[] = "行 {$rowIndex}, セル {$cellIndex}: file_display タイプは配列形式である必要があります";
                }
                break;
        }
