...
class ValueFormatterTest extends TestCase
{
    ...
    public function it_formats_file_links_correctly()
    {
        ...
        $expected = '<a href="/path/to/document.pdf" class="text-decoration-none" target="_blank"><i class="fas fa-file-pdf text-danger"></i> document.pdf</a>';
        $this->assertEquals($expected, $result);
        
        // Image file
        $result = ValueFormatter::format('/path/to/image.jpg', 'file');
        $expected = '<a href="/path/to/image.jpg" class="text-decoration-none" target="_blank"><i class="fas fa-file-image text-info"></i> image.jpg</a>';
        $this->assertEquals($expected, $result);
        
        // Custom display name
        $result = ValueFormatter::format('/path/to/document.pdf', 'file', ['display_name' => 'My Document']);
        $expected = '<a href="/path/to/document.pdf" class="text-decoration-none" target="_blank"><i class="fas fa-file-pdf text-danger"></i> My Document</a>';
        $this->assertEquals($expected, $result);
        
        // Unknown file type
        $result = ValueFormatter::format('/path/to/file.unknown', 'file');
        $expected = '<a href="/path/to/file.unknown" class="text-decoration-none" target="_blank"><i class="fas fa-file text-muted"></i> file.unknown</a>';
        $this->assertEquals($expected, $result);
    }
    ...
    public function it_formats_using_format_method_with_different_types()
    {
        ...
        $testCases = [
            ...
            ['value' => '/path/to/file.pdf', 'type' => 'file', 'expected' => '<a href="/path/to/file.pdf" class="text-decoration-none" target="_blank"><i class="fas fa-file-pdf text-danger"></i> file.pdf</a>'],
        ];
