...
import { DocumentManager } from './modules/document-management.js';
...
class Application {
  ...
  /**
   * Initialize document management for facility pages
   */
  async initializeDocumentManagement(facilityId) {
    try {
      // Check if document management container exists
      ...
      // Get configuration from global window object (set by view)
      const config = window.documentManagementConfig || {};

      console.log('Document management config:', config);

      // Initialize document manager
      const { DocumentManager } = await import('./modules/document-management.js');
      const documentManager = new DocumentManager({
        facilityId: config.facilityId || facilityId,
      ...
      // Store in app state
      appState.setModule('documentManager', documentManager);

      // Initialize immediately if on documents page, otherwise wait for tab
      const isDocumentsPage = window.location.pathname.includes('/documents');
      if (isDocumentsPage) {
        await documentManager.init();
        documentManager.initialized = true;
        console.log('Document management initialized immediately for documents page');
      } else {
        // Initialize when documents tab is shown
        ...
        if (documentsTab) {
          documentsTab.addEventListener('shown.bs.tab', async function () {
            if (!documentManager.initialized) {
              await documentManager.init();
              documentManager.initialized = true;
              console.log('Document management initialized via tab activation');
            }
          });
        }
      }
